...
2. **RealDash installed and able to reach the editor.**
   ...
   - Credentials for the My RealDash account are in your own RealDash account credentials (never commit them). The account has an active
...
1. Install RealDash (section 0.2) and launch it once. Log in with the your own RealDash account credentials (never commit them) account when
...
- On first launch RealDash shows a language dialog (pick English -> DONE), then a **My RealDash**
  login (email + password). Use the account in your own RealDash account credentials (never commit them).
