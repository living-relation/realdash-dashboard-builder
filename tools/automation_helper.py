...
# Make this process DPI-aware BEFORE importing pyautogui so that screen
# coordinates, screenshots, and clicks all operate in true physical pixels.
# Without this, on a scaled display (e.g. 150%/200%) the DPI-unaware screenshot
# and cursor coordinates disagree with the real on-screen positions and clicks
# miss their targets.
try:
    import ctypes
    try:
...
def main(argv):
    ...
    if cmd == "key":
        presses = int(a[1]) if len(a) > 1 else 1
        pyautogui.press(a[0], presses=presses, interval=0.02)
        print("pressed %s x%d" % (a[0], presses))
        return 0
